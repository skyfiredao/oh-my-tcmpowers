...
辅行诀体系中的方剂五行结构分析。输入药方后，前置加载 omtp-core-fxj，逐药定位五行坐标（第一层味属+第二层行中之行），然后统计整方五行结构特征。不涉及路由判断、配伍比例分析或框架推导。与 omtp-analyze-zhengzhuang 互为反向流程。
...
### 药表外处理（Level 2 / Level 3）

药方中出现不在 60 味互含表内的药物，按 TaoHongJing 三级 fallback 处理：

- **Level 1**：互含表 60 味直接查得（本步骤不适用）
- **Level 2**：查 `data/bencao/<药名>.md`（340 味历代本草），按性味归经推导五行坐标
- **Level 3**：本地 bencao 无对应文件时，网络搜索 + 感官习性综合判断

**Level 3 判断维度**（综合，无固定优先级）：
...
**判断过程必须展示**：标注当前层级（Level 2 / Level 3）+ 信息来源（`data/bencao/<药名>.md` 或网络搜索），说明综合判断依据，再给出五行定位结论。
...
- 跳过60味药表直接查 bencao -- 先查互含表，不在表中才查 `data/bencao/`
- 跳过本地 bencao 直接网络搜索 -- 本地 340味本草数据优先，无对应文件才允许网络搜索
