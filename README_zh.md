...
一套 AI 技能集 + agent 编排层，基于多种经典中医理论体系，对中医方剂、症状、脉象与针灸穴位进行结构化分析。5 个 agent 负责入口路由、上下文隔离与并行调度，17 个 skill 提供纯领域知识，覆盖核心理论、组方规则、针灸取穴规则、脉象诊断和分析方法。症状分析时同时推导辅行诀草药方剂、圆运动处方、辨证录翻案处方和针灸取穴组合，形成四路径交叉验证；脉诊提供独立的逆向验证通道。

## 功能

中医之心（Oh My TCM Powers）提供了一套结构化、规则驱动的分析框架，用辅行诀理论体系分析方剂、临床症状和针灸取穴。通过五行生克、体用化分层和量化组方规则来推导治疗机理。

- **5 个 agent + 17 个 skill**，agent 负责路由和编排，skill 负责纯领域知识
...
| 《辨证录》 | 陈士铎 | 清 | 14卷126门辨证翻案体系："似X非X"鉴别法、补法为主处方、反佐配伍、大剂量多脏同调 |
