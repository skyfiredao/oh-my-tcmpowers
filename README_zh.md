...
一套 AI 技能集 + agent 编排层，基于多种经典中医理论体系，对中医方剂、症状、脉象与针灸穴位进行结构化分析。5 个 agent 负责入口路由、上下文隔离与并行调度，18 个 skill 提供纯领域知识，覆盖核心理论、组方规则、针灸取穴规则、脉象诊断和分析方法。症状分析时同时推导辅行诀草药方剂、圆运动处方、辨证录翻案处方和针灸取穴组合，形成四路径交叉验证；脉诊提供独立的逆向验证通道。

## 功能

中医之心（Oh My TCM Powers）提供了一套结构化、规则驱动的分析框架，用辅行诀理论体系分析方剂、临床症状和针灸取穴。通过五行生克、体用化分层和量化组方规则来推导治疗机理。

- **5 个 agent + 18 个 skill**，agent 负责路由和编排，skill 负责纯领域知识
- **三层架构**：agent（路由/编排/档案）→ skill（核心理论/规则/分析方法）→ data（药物表/穴位表/症状数据）
- **并行四链路分析**：辨症分析同时推导辅行诀草药方剂、圆运动处方、辨证录翻案处方和针灸取穴组合，收敛/分歧对比
...
- **圆运动子系统**：彭子益升降浮沉模型，含 8 个基本病机型（+亚型）、15 种复合型、10 种组方规则型、约 120 味按气机功能分类的药物
...
| `omtp-agent-TaoHongJing`（陶弘景） | 中药专家（辅行诀）。加载 core-fxj，按分析结果路由到规则分支，执行完整推导链 |
| `omtp-agent-PengZiYi`（彭子益） | 圆运动专家。分层加载：辨识阶段加载 core-yyd + data/yyd-bianshi.md，组方阶段加载 rules-yyd + data/yyd-zufang.md + data/yyd-yaoxing.md |
| `omtp-agent-ChenShiDuo`（陈士铎） | 辨证录专家。加载 core-bzl → rules-bzl，"似X非X"辨证翻案法，补法为主、反佐配伍、大剂量多脏同调 |
...
| `omtp-core-yyd` | 圆运动核心理论框架：彭子益升降浮沉模型、中气轴心、以偏纠偏总则、8型速查表（详细辨识数据见 data/yyd-bianshi.md） |
| `omtp-core-bzl` | 辨证录辨证方法论：论→辨→治→方四步链、"似X非X"翻案逻辑、126门路由表 |
...
| `omtp-rules-yyd` | 圆运动组方规则框架：组方三原则、三层结构、10型速查、剂量比例原则（详细组方数据见 data/yyd-zufang.md） |
| `omtp-rules-bzl` | 辨证录组方规则：补法为主、反佐法、大剂量策略、多脏同调、3级 fallback（精确匹配→原文方加减→框架推导） |
...
Agent 层 (路由 + 编排 + 档案管理)
  扁鹊 (BianQue)                 # 全科专家、诊断、派发、汇总
    ├── 陶弘景 (TaoHongJing)     # 中药专家（辅行诀）
    ├── 彭子益 (PengZiYi)        # 圆运动专家
    ├── 陈士铎 (ChenShiDuo)      # 辨证录专家
    └── 皇甫谧 (HuangFuMi)       # 针灸专家
...
Data 层 (只读，按次 symlink)
  ...
  yyd-bianshi.md                # 圆运动辨识数据（8型详解+亚型+鉴别+相火+复合型）
  yyd-zufang.md                 # 圆运动组方数据（10型分治+复合型模板+示例）
  yyd-yaoxing.md                # 圆运动药性提纲篇（约120味药）
  bzl/*.md                      # 辨证录126门病案数据（按门分文件）
  zj-12zj-*.md                  # 十二正经穴位数据
  zj-qj8m-*.md                  # 奇经八脉穴位数据
```
...
| `omtp-agent-TaoHongJing`（陶弘景） | 中药专家（辅行诀）：加载 core-fxj，路由到唯一规则分支（fxj5z/fxj1xz/fxj2d6s），执行完整推导链 |
| `omtp-agent-PengZiYi`（彭子益） | 圆运动专家：分层加载 core-yyd → bianshi → rules-yyd → zufang → yaoxing，独立从症状分析升降失常型别并推导组方 |
| `omtp-agent-ChenShiDuo`（陈士铎） | 辨证录专家：加载 core-bzl → rules-bzl，"似X非X"辨证翻案法，补法为主、反佐配伍、大剂量多脏同调 |
| `omtp-agent-HuangFuMi`（皇甫谧） | 针灸专家：加载 core-zj + rules-zj + rules-ziwu-liuzhu，脏→经络映射，12 法取穴 + 子午流注 |

上下文隔离：各专家 agent 禁止加载其他专家的资源。陶弘景、彭子益、陈士铎、皇甫谧独立完成推导，结论由 router 汇总对照。
...
|---|---|---|
| **zhengzhuang（症状）** | 主诉、症状描述、病程叙述 | 生成共享分析契约，再**并行**派发 TaoHongJing + PengZiYi + ChenShiDuo + HuangFuMi |
...
`frontend/` 目录包含一个单文件 Web 问诊界面，连接 OpenCode HTTP API。用户通过交互式问诊单提交症状，系统调度中医分析智能体并实时回传分析结果。

详见 **[frontend/README_zh.md](frontend/README_zh.md)**（启动方式、网络配置、使用说明）。([English](frontend/README.md))
...
│   ├── omtp-agent-TaoHongJing.md               # 陶弘景 中药专家（辅行诀）
│   ├── omtp-agent-PengZiYi.md                  # 彭子益 圆运动专家
│   ├── omtp-agent-ChenShiDuo.md                # 陈士铎 辨证录专家
│   └── omtp-agent-HuangFuMi.md                 # 皇甫谧 针灸专家
├── data/
│   ├── fxj-zhengzhuang.md                      # 症状分析基准数据
│   ├── fxj-maixiang.md                         # 脉象诊断参考数据（27脉）
│   ├── maizhen-maijue.md                       # 陈士铎《脉诀阐微》：兼见脉、部位症候、生死脉
│   ├── shanghan-liujing.md                     # 六经→二旦六神方剂映射
│   ├── wenbing-wei-qi-ying-xue.md              # 温病四级分期数据
│   ├── wenbing-sanjiao.md                      # 温病三焦分期数据
│   ├── yyd-bianshi.md                          # 圆运动辨识数据（8型详解+亚型+鉴别+相火+复合型）
│   ├── yyd-zufang/                             # 圆运动组方数据（按病种分文件）
│   ├── yyd-yaoxing.md                          # 圆运动药性提纲篇（约120味药）
│   ├── bzl/                                    # 辨证录125门病案数据（按门分文件 + 凡例 + 序跋）
...
├── frontend/
│   ├── index.html                              # Web 问诊界面（单文件 HTML+CSS+JS）
│   ├── wenzhendan.json                         # 问诊单数据（19 个症状分类）
│   ├── README.md                               # 前端文档（英文）
│   └── README_zh.md                            # 前端文档（中文）
└── skills/
    ├── omtp-core-fxj/                          # 核心理论（五行、体用化、60味药表、化味合成）
    ├── omtp-core-yyd/                          # 圆运动核心理论框架（升降模型、中气轴心、8型速查）
    ├── omtp-core-bzl/                          # 辨证录方法论（论→辨→治→方、126门路由）
    ...
    ├── omtp-rules-yyd/                         # 圆运动组方规则框架（组方三原则、三层结构、10型速查）
    ├── omtp-rules-bzl/                         # 辨证录组方规则（补法+反佐+大剂量+多脏同调）
...
中医之心（Oh My TCM Powers）与其他技能集完全独立。所有技能以 `omtp-` 为前缀，通过符号链接安装，与编程技能（Superpowers）和写作技能（Dreampowers）互不干扰。
...
| 《圆运动的古中医学》 | 彭子益 | 民国 | 升降浮沉圆运动模型、中气轴心理论、8 型病机辨识、药物气机功能分类 |
| 《辨证录》 | 陈士铎 | 清 | 14卷125门辨证翻案体系："似X非X"鉴别法、补法为主处方、反佐配伍、大剂量多脏同调 |
| 《脉诀阐微》 | 陈士铎 | 清 | 五篇脉诀：38字脉诀总纲、兼见脉组合诊断、寸关尺×脉象→具体症候、生死脉预后判断 |
| 《历代本草药性汇解》 | 综合 | 历代 | 340味中药性味归经汇编，汇集历代本草文献评述 |
