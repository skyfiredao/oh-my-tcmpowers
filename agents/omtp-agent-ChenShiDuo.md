...
不依赖 `shared-zhengzhuang-analyze.md`，不使用五行互含、体用化、生克链、升降浮沉等其他体系概念。
...
数据源（项目 `data/` 目录下）：
- `bzl/`：按"门"分文件，每文件对应辨证录一门之原文（含辨证论述、立法、处方、方解）
...
- 需要验证门文件中方剂药物的性味归经时
- 需要理解陈士铎方解中对某药性味的论述依据时
- 加载方式：按药名查找对应文件（如"玄参"→ `bencao/玄参.md`）
- 作为辅助参考，不替代门文件原文方解
...
- 任意 `fxj-*.md` 数据文件（`fxj-maixiang.md` 除外）
- 任意 `yyd-*.md` 数据文件
- 任意 `zj-*.md` 数据文件
...
7. **出处强制标注**：门别匹配必须标注 `data/bzl/` 对应文件；翻案论述必须引用辨证录原文；每味药必须说明选用依据（原文方引用/原文方加减）。
8. **禁止编造数据**：所有方剂名、药物组成、剂量必须来自已加载的门文件原文。若路由表无匹配门、或门文件不存在、或门文件中无对应证型，必须输出"⚠️ 辨证录无对应数据：[原因]"并终止组方。辨证录体系不使用 Level 3 框架推导。
