...
- 药物不在 `omtp-core-fxj` 五行互含表 60 味范围内
- 用于 Level 2 性味推导（取 bencao 的性味归经，按五味→五行规则定位）
- 加载方式：按药名查找对应文件（如"黄芪"→ `bencao/黄芪.md`；炮制变体和异体字通过 symlink 别名自动解析）
...
7. **禁止编造数据**：所有药物五行定位优先来自 `omtp-core-fxj` 的五行互含表（60味药）。若药物不在互含表中，按以下三级 fallback：
   - Level 1：互含表直接查得 → 直接使用
   - Level 2：查 `data/bencao/<药名>.md` 获取性味归经 → 按五味→五行规则推导五行坐标，标注"Level 2: bencao 数据 + 五味推导"
   - Level 3：本地 bencao 无对应文件 → 调用 `omtp-analyze-fangji` 的"药表外处理"流程（网络搜索 + 感官习性综合判断），标注"Level 3: 框架推导"
   - 禁止跳过 Level 1/2 直接进入 Level 3，禁止从模型训练知识中凭空补充五行归属。组方规则必须来自已加载的规则分支 skill。
